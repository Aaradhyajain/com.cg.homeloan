...
public class LandVerificationOfficerController {
	...
	public ResponseEntity<LoanApplication> updateLandStatus(@RequestBody LoanApplication loanApplication)
		...
		return new ResponseEntity<LoanApplication>(landVerificationService.updateStatus(loanApplication),
				HttpStatus.OK);
	...
	public ResponseEntity<Boolean> isValidLandOfficer(@PathVariable String username, @PathVariable String password) {
		return new ResponseEntity<Boolean>(landVerificationService.isValidLandOfficer(username, password),
				HttpStatus.OK);
	}
